...
function selectTemplate(days: TrainingDays, exp: Experience): TemplateResult {
  console.log('[ProgrammeBuilder] selectTemplate called: days =', days, '| exp =', exp);
...
function getKeyNote(goal: Goal): string {
  const notes: Record<Goal, string> = {
    ...
    fitness: 'Consistency beats everything. Showing up 3-4 times a week for 6 months beats any perfect programme followed for 3 weeks.',
  };
  return notes[goal];
}
...
function convertSession(
  ...
  const exercises: ProgrammeExercise[] = session.exercises.map((ex) => {
    let sets = ex.sets;
    if (female) sets += 1;
...
// ─── Profile Goal Mapper ──────────────────────────────────────────────
// Maps onboarding goal strings → programme builder Goal type
